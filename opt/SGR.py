...
class risk_control:
    ...
    def bound(self,rstar,delta,kappa,residuals,split=True):
        ...
        if split:
            print("%.2f & train_risk=%.4f & train_cov=%.4f & test_risk=%.4f & test_cov=%.4f & bound=%.4f  \\\\" % (rstar,risk,coverage,testrisk,testcov,bound))
        else:
            print("SGR: rstar=%.2f & r=%.4f & cov=%.4f & rbound=%.4f & quantile=%.2f  \\\\" % (rstar,risk,coverage,bound, mid/float(m)))
        return [theta,bound]