...
def __loss_class_specific_helper(errs, sure_cnts, total_unsure, alphas, class_weights, N, la, lc, lcs):
    ambiguity_loss = total_unsure / N * la
    if sure_cnts.min() == 0: return np.inf
    risks = errs / sure_cnts
    tempf = risks - alphas
    if class_weights is None:
        coverage_loss = np.power(tempf.clip(0, 1), 2).sum() * lc
        coverage_loss_sim = np.power(tempf, 2).sum() * lcs
    else:
        coverage_loss = np.dot(np.power(tempf.clip(0, 1), 2), class_weights) * lc
        coverage_loss_sim = np.dot(np.power(tempf, 2), class_weights) * lcs
    loss = ambiguity_loss + coverage_loss + coverage_loss_sim
    return loss
